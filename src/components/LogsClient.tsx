...
export function LogsClient({ initialLogs }: LogsClientProps) {
  ...
  function handleToggleLiveSync() {
    setLiveSync((value) => {
      const next = !value;
      showToast(next ? "Live sync started" : "Live sync stopped", next ? "success" : "info");
      return next;
    });
  }
          ...
          <div className="flex space-x-3">
            ...
            >
              <span className="relative flex h-2.5 w-2.5">
                ...
                <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${liveSync ? "bg-emerald-500" : "bg-zinc-500"}`} />
              </span>
              <span>{liveSync ? "Live Sync On" : "Live Sync Off"}</span>
            </button>
          </div>
        </div>
